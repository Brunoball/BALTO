    ...
    <div ref={overlayRef} className="gm-modal-overlay" data-servicios-modal-overlay="true" onMouseDown={cerrarDesdeFondo}>
      <form className="gm-modal-container gm-modal-container--small gm-modal-v2 servicios-modal" onSubmit={submit} role="dialog" aria-modal="true">
        ...
        <div className="gm-modal-content servicios-modal__content"><div className="servicios-form-grid">
          <label className="gm-field servicios-field--span-7"><input className="gm-input" autoFocus maxLength={80} value={form.nombre} onChange={(e) => setForm((p) => ({ ...p, nombre: clampText(e.target.value, 80) }))} placeholder=" " /><span className="gm-label">Nombre</span></label>
          <label className="gm-field servicios-field--span-5"><input className="gm-input" maxLength={20} value={form.simbolo} onChange={(e) => setForm((p) => ({ ...p, simbolo: clampText(e.target.value, 20) }))} placeholder=" " /><span className="gm-label">Símbolo</span></label>
        </div></div>
