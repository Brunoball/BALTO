...
export default function ModalMedioPago({ open, item, saving, onClose, onSave, onToast }) {
  ...
  const submit = async (event) => {
    ...
    if (!limpio) {
      onToast?.("error", "Completá el nombre del medio de pago.", 4200);
      return;
    }
  ...
  return createPortal(
    <div className="gm-modal-overlay" data-modal-overlay="true">
      <form className="gm-modal-container gm-modal-container--small gm-modal-v2" onSubmit={submit} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
        <header className="gm-modal-header">
          <div className="gm-modal-head-left">
            ...
            <p className="gm-modal-subtitle">Estará disponible en los formularios de cobro y pago del sistema.</p>
          </div>
