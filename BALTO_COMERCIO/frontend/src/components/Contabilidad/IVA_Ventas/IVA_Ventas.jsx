...
function getCliente(row) {
  return String(row?.cliente || row?.entidad || row?.razon_social || "").trim() || "SIN CLIENTE";
}
...
export default function IVAVentas() {
  ...
  const exportBaseName = useMemo(() => {
    if (range.from && range.to) return `iva_ventas_${dateToAPI(range.from)}_${dateToAPI(range.to)}`;
    if (range.from) return `iva_ventas_desde_${dateToAPI(range.from)}`;
    if (range.to) return `iva_ventas_hasta_${dateToAPI(range.to)}`;
    return "iva_ventas_todos";
  }, [range.from, range.to]);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, slugifySheetName("IVA_Ventas"));
  ...
  const emptyText = loading
    ? "Cargando registros de IVA ventas..."
    : error || (q.trim() ? `No se encontraron ventas para "${q.trim()}".` : "No hay registros de IVA ventas para mostrar en el rango seleccionado.");
        ...
        <div className="mov-card__head">
          <div className="mov-card__headLeft">
            <div className="title-mov">
              <div className="mov-card__title">IVA Ventas</div>
              <div className="mov-card__hint">
                Mostrando <b>{filteredRegistros.length}</b> registros de IVA ventas
