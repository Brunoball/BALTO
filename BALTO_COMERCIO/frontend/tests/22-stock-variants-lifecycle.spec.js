...
test('@stock @crud variantes: alta, atributos, baja, reactivación y eliminación', async ({ page }) => {
  ...
  await variantCard.locator('.cmi-floatingField, .fl-field').filter({ hasText: /^Stock/i }).locator('input').first().fill('6');
  ...
  await editVariantCard.locator('.cmi-floatingField, .fl-field').filter({ hasText: /^Stock/i }).locator('input').first().fill('8');
