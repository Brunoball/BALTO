...
const PAGE_ONE = [
  {
    ...
    items_detalle: [
      {
        tipo_item_db: 'PRODUCTO',
        stock_producto_nombre: 'Remera Export E2E',
        stock_variante_nombre: 'Azul M',
        stock_variante_sku: 'EXP-REM-AZM',
      ...
      {
        tipo_item_db: 'PRODUCTO',
        stock_producto_nombre: 'Cable Export E2E',
        sku: 'EXP-CAB-01',
        cantidad: 2,
        precio: 50,
        subtotal: 100,
  ...
  },
  {
    ...
    items_detalle: [
      {
        tipo_item_db: 'PRODUCTO',
        stock_producto_nombre: 'Producto Compra Export E2E',
        cantidad: 4,
        precio: 20,
        subtotal: 80,
        iva_pct: 0,
...
test('@critical @export movimientos exporta sólo la página visible cuando se elige "esta página"', async ({ page }, testInfo) => {
  ...
  expect(csv).toContain('Remera Export E2E');
  expect(csv).toContain('Azul M');
  expect(csv).toContain('Cable Export E2E');
...
test('@critical @export movimientos exporta todas las páginas y Excel incluye el detalle completo en hojas separadas', async ({ page }, testInfo) => {
  ...
  expect(String(venta.DETALLE)).toContain('Remera Export E2E');
  expect(String(venta.DETALLE)).toContain('Azul M');
  expect(String(venta.DETALLE)).toContain('Cable Export E2E');
  expect(String(venta.DETALLE)).not.toBe('2 PRODUCTOS');

  expect(items.some((row) => String(row.ITEM).includes('Remera Export E2E') && String(row.ITEM).includes('Azul M') && row.VARIANTE === 'Azul M' && row.SKU === 'EXP-REM-AZM' && String(row.CODIGO_BARRAS) === '7791234567890')).toBe(true);
  expect(items.some((row) => row.ITEM === 'Cable Export E2E' && row.SKU === 'EXP-CAB-01')).toBe(true);
...
test('@export integración: las pantallas paginadas conservan el contrato página/todos', async () => {
  ...
  const movimientosUtilsSource = await fs.readFile(
    path.join(root, 'src/components/Movimientos/utils/movimientosUtils.js'),
    'utf8',
  );
  expect(movimientosUtilsSource).toContain('VARIANTE:');
  expect(movimientosUtilsSource).toContain('CODIGO_BARRAS:');
  expect(movimientosUtilsSource).toContain('stock_variante_nombre');
  expect(movimientosUtilsSource).toContain('stock_producto_nombre');
